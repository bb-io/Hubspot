...
public class MarketingEmailService(InvocationContext invocationContext) : BaseContentService(invocationContext)
{
    ...
    public override async Task<Metadata> UpdateContentFromHtmlAsync(string targetLanguage, Stream stream, UploadContentRequest uploadContentRequest)
    {
        ...
        if (uploadContentRequest.CreateNew != true && blackbirdId == null)
        {
        ...
        {
            return await CreateNewEmailFromHtmlAsync(titleText, language ?? targetLanguage, businessUnitId, updatedContent);
        }
        else
        {
    ...
    private async Task<Metadata> CreateNewEmailFromHtmlAsync(string? titleText, string language, string? businessUnitId, Content updatedContent)
    {
        var requestPayload = new MarketingEmailOptionalRequest
        {
            Name = titleText ?? "New Marketing Email",
            Language = language,
        ...
        if (!string.IsNullOrEmpty(requestPayload.BusinessUnitId))
        {
            createRequest.AddQueryParameter("businessUnitId", requestPayload.BusinessUnitId);
        }

        var emailDto = await Client.ExecuteWithErrorHandling<MarketingEmailDto>(createRequest);
